...
/-
The "def" keyword introduces a new definition. We give a name to our "function," namely "id_boolean". This function takes a single value, b, of type boolean, and returns value
of type boolean. You can read this definition as saying that, "When id_boolean is applied 
to an argument, b, of type boolean, the result is a value of type boolean. In particular, the result that is returned is given by the expression to the right of the :=. That is 
just b. So the value that the function returns is just the value to which it was applied.
And that makes this a valid representation/implementation of the identity function. 
-/
...
/-
Test cases for this function. A test case asserts that the result actually obtained by applying some function to arguments is the result that is expected if the function definition is correct. Here's a test case for id_bool. It's a little "putative theorem"
called T_id_T that asserts that the result of applying id_boolean to T is T. To the right
of the := is a proof: the term "rfl". This term serves as a proof that any value is equal 
to itself, and it takes care of reducing expressions to values before deciding whether it
sees "the same" value on each side of the = sign. Here, the application of id_boolean to T
reduces to T, and T = T, so rfl is happy to serve as a proof that id_boolean T = T. 
-/
